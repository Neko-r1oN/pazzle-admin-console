    ...
    trait InteractsWithDockerComposeServices
    {
        ...
        protected function buildDockerCompose(array $services)
        {
            ...
            if (!array_key_exists('laravel.test', $compose['services'])) {
                $this->warn('Couldn\'t find the laravel.test service. Make sure you add [' . implode(',',
                        $services) . '] to the depends_on config.');
            } else {
            ...
            collect($services)
                ->filter(function ($service) {
                    return in_array($service,
                        ['mysql', 'pgsql', 'mariadb', 'redis', 'meilisearch', 'typesense', 'minio']);
                })->filter(function ($service) use ($compose) {
            ...
            file_put_contents($this->laravel->basePath('docker-compose.yml'),
                Yaml::dump($compose, Yaml::DUMP_OBJECT_AS_MAP));
        }
        ...
        protected function configurePhpUnit()
        {
            ...
            $phpunit = str_replace('<!-- <env name="DB_DATABASE" value=":memory:"/> -->',
                '<env name="DB_DATABASE" value="testing"/>', $phpunit);
        ...
        protected function prepareInstallation($services)
        {
            ...
            if (count($services) > 0) {
                $this->runCommands([
                    './vendor_/bin/sail pull ' . implode(' ', $services),
                ]);
            }

            $this->runCommands([
                './vendor_/bin/sail build',
            ]);
        }
