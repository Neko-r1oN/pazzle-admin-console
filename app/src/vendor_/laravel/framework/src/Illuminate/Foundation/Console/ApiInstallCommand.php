<?php
    ...
    class ApiInstallCommand extends Command
    {
        ...
        public function handle()
        {
            ...
            if ($this->option('passport')) {
                ...
                if (!$this->option('without-migration-prompt')) {
                    if ($this->confirm('One new database migration has been published. Would you like to run all pending database migrations?',
                        true)) {
                        $this->call('migrate');
                    }
                }
        ...
        protected function installSanctum()
        {
            ...
            $migrationPublished = collect(scandir($this->laravel->databasePath('migrations')))->contains(function (
                $migration
            ) {
